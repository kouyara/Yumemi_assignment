...
function App() {
	...
	const [prefecturePopulations, setPrefecturePopulations] = useState<
	...
	const shouldFetch = useCallback(
		(prefCode: number) =>
			!prefecturePopulations.some((p) => p.prefCode === prefCode),
	...
	useEffect(() => {
		...
		for (const prefCode of selectedPrefectures) {
			if (shouldFetch(prefCode)) fetchPopulationData(prefCode);
		}

		setPrefecturePopulations((prev) =>
			prev.filter((pref) => selectedPrefectures.includes(pref.prefCode)),
		);
	}, [selectedPrefectures, shouldFetch, prefecturePopulations, prefectures]);
	...
	const getChartOptions = () => {
		const years =
			prefecturePopulations[0]?.data[0]?.data.map((item) => item.year) || [];
		return {
